...
sh_array_push(struct sh_array *self) {
    if (self->nelem == self->cap) {
        self->cap *= 2;
        self->elem = sh_realloc(self->elem, self->size * self->cap);
    }
...
static inline void
sh_array_remove(struct sh_array *self, size_t idx) {
    assert(idx < self->nelem);
    uint8_t *p = (uint8_t *)self->elem + self->size * idx;
    int i;
    for (i=idx; i<self->nelem-1; ++i) {
        memcpy(p, p+self->size, self->size);
        p+=self->size;
    }
    --self->nelem;
}
