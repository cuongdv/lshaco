...
static int
lrsa_private_encrypt(lua_State *L) {
    RSA *rsa = torsa(L);
    size_t l;
...
static int
lrsa_public_decrypt(lua_State *L) {
    RSA *rsa = torsa(L);
    size_t l;
    const char * str = luaL_checklstring(L,2,&l);
    if (l==0) {
...
static void
rsa_createmeta(struct lua_State *L) {
    if (luaL_newmetatable(L, "RSA*")) {
        luaL_Reg l[] = {
            { "private_encrypt", lrsa_private_encrypt },
            { "public_decrypt",  lrsa_public_decrypt },
            {"__gc", lrsa_free },
            {NULL, NULL},
        };
        lua_pushvalue(L, -1);
...
static int
lrsa_new(lua_State *L) {
    const char *file = luaL_checkstring(L,1);
    luaL_checktype(L,2,LUA_TBOOLEAN);
    int public = lua_toboolean(L,2);
    
    BIO *out = BIO_new_file(file, "rb");
    if (out == NULL) {
        return luaL_error(L, "Open rsa key file fail:%s", file);
    }
    RSA* rsa;
    if (public) {
    ...
    if (rsa == NULL) {
        return luaL_error(L, "Read rsa key error, in %s", file);
    }
    BIO_free(out);

    RSA **prsa = lua_newuserdata(L, sizeof(RSA*));
    *prsa = rsa;
    rsa_createmeta(L);    
