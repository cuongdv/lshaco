...
function _decode_type_meta:__index(key)
	...
	--local func = assert(_reader[t],string.format("not found reader:%s.%s [t:%d]", self._CType, key, t))(msg)
	self[key] = func
	return func
end
...
function _encode_type_meta:__index(key)
    ...
    --local func = assert(_writer[t],string.format("not found writer:%s.%s [t:%d]", self._CType, key, t))(msg)
	self[key] = func
	return func
end
