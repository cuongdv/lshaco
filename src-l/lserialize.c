...
static int pack_one(lua_State *L, struct write_block *b, int index, int depth);
...
pack_one(lua_State *L, struct write_block *b, int index, int depth) {
	...
	case LUA_TLIGHTUSERDATA:
        if (lua_type(L,index+1) != LUA_TNUMBER) {
            luaL_error(L, "lightuserdata must followed by number");
        }
        int sz = lua_tointeger(L, index+1);
...
*/
static void
get_mem(lua_State *L, struct read_block *rb, int sz) {
    void * tmp = 0;
	void * v = rb_read(rb, &tmp, sz);
	if (v == NULL) {
		invalid_stream(L,rb);
	}
